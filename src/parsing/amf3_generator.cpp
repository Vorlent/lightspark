			...
			std::vector<TraitsRef>& traitsMap) const
{
	...
	if((objRef&0x07)==0x07)
	{
		...
		asAtom ret= type->getInstance(true, empty, 0);
		//Invoke readExternal
	...
	std::vector<asAtom> empty;
	asAtom ret=(traits.type)?traits.type->getInstance(true, empty, 0):
